      ...
      this.rendering();
            ...
            if (this.state.isReady == false) {
              ...
              if (this.state.call == true) {  
                if (this.state.getClusters == true) {  
                this.props.getBlockClusters(this.props.block);
                //So we call the sets only once
                this.setState({ getClusters: false })
                }
                if (this.props.clusters.length > 0 && this.props.clusters[0].block == this.props.block) {
                  this.setState({ clusters: this.props.clusters })
                  this.setState({ call: false })
                }
            }
            //Load anyways after 10sec if there's no clusters
              setTimeout(() => this.setState({ call: false }), 10000);
            if (this.state.call == false) {       
              if ( this.state.selectedView == this.props.block) {
                setTimeout(() => this.setState({ sets: this.props.clusters }), 500);
                setTimeout(() => this.setState({ isReady: true}), 1000);
              } else {
                setTimeout(() => this.setState({ isReady: true, subject: this.state.selectedView, clusters: this.props.clusters.filter((cluster) =>  cluster.subject == this.state.selectedView )}), 1000);
              }
            }
          }
...
{this.state.selectedView == "Genitourinary" && (
          ...
          {user
            ? this.props.auth.user.role &&
              this.props.auth.user.role == "Educator" && (
                <Button
